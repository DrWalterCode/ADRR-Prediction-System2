...
End Sub
    ...
    Private Sub TileItem4_ItemClick(sender As Object, e As DevExpress.XtraEditors.TileItemEventArgs) Handles TileItem4.ItemClick
       ...
       End Try
    End Sub
    Public Sub LoadgridAllpatients()
    Me._con.Close
    Try 
        Dim cmdText As String = "select * from patient_view "
        Me._cmd = New MySqlCommand(cmdText)
        Dim data As DataTable = New InsertUpdateData().GetData(Me._cmd)
        Me.dgvpatients.DataSource = data
        Me.dgvpatients.FirstDisplayedScrollingRowIndex = (Me.dgvpatients.RowCount - 1)
        Me.dgvpatients.Rows.Item((Me.dgvpatients.RowCount - 1)).Selected = True
        Me.dgvpatients.CurrentCell = Me.dgvpatients.Rows.Item((Me.dgvpatients.RowCount - 1)).Cells.Item(0)
        Me._con.Close
    Catch exception1 As Exception
        ProjectData.SetProjectError(exception1)
        Dim exception As Exception = exception1
